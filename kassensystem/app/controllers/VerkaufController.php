...
class VerkaufController extends \Phalcon\Mvc\Controller

{
    ...
    public function overviewAction($number_page = 1, $items_per_page = 10) {
        ...
        FROM Warenrevisionen, Waren WHERE Warenrevisionen.revision = Waren.cur_rev AND Warenrevisionen.id = Waren.id AND Waren.id LIKE :id:", ["id" => "%".$this->request->get('id', 'int')."%"]);
        else
        $waren = $this->modelsManager->executeQuery("SELECT Warenrevisionen.id, price, mehrwertsteuer_voll, description, deleted, created 
        FROM Warenrevisionen, Waren WHERE Warenrevisionen.revision = Waren.cur_rev AND Warenrevisionen.id = Waren.id");
    ...
    public function addAction() {
        $this->dispatcher->forward(["verkauf/edit/"]);
    }
...
*/
    ...
    public function editAction($id = -1) {
        ...
        if ($this->request->isPost()) {
            ...
            $w->revision = $rev;
            $x->cur_rev = $rev;
            $w->price = $this->request->getPost("preis", "float");
            ...
            $ware = $this->modelsManager->executeQuery("SELECT Warenrevisionen.id, price, mehrwertsteuer_voll, description
